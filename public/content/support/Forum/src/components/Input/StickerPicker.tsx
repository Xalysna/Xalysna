          ...
          {loading || error ? (
            ...
            <div className="flex h-full w-full flex-col">
                ...
                {recentStickers.length > 0 && (
                  <>
                    ...
                    <div className="grid w-full grid-cols-5 justify-between">
                      {recentStickers.map((url, index) => (
                        <SpriteRenderer
                          key={index} // Agrega una clave única para cada sticker reciente
                          size={60}
                          onClick={() => {
                            onSelect(url);
                  ...
                  <Fragment key={collection.id}>
                    ...
                    <div className="grid w-full grid-cols-5 justify-between">
                      {collection.stickers.map((sticker, index) => (
                        <SpriteRenderer
                          key={index} // Agrega una clave única para cada sticker
                          size={60}
                          onClick={() => {
              ...
              <div className="h-18 border-t-dark-lighten flex w-full flex-shrink-0 gap-2 overflow-x-auto border-t p-2">
                ...
                {data?.map((collection) => (
                  <img
                    key={collection.id} // Agrega una clave única para cada imagen de colección
                    onClick={() =>
                      document
