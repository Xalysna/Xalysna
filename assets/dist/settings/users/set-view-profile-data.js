...
// Función auxiliar para establecer el valor de un campo de texto multilínea (textarea)
function setTextareaValue(elementId, value) {
    const textarea = document.getElementById(elementId);

    if (textarea && textarea.tagName.toLowerCase() === 'textarea') {
        textarea.textContent = value || ''; // Asignar el valor o una cadena vacía si es nulo/undefined
        console.log(`%c✅ Valor '${value}' establecido en el textarea con ID ${elementId}.`, 'color: #2ecc71'); // Verde claro
    } else {
        console.warn(`%c❌ Elemento con ID ${elementId} no encontrado o no es un textarea.`, 'color: #e74c3c'); // Rojo claro
    }
}
          ...
          rellenarNombresSpan(userData);
            ...
            switch (userData.tipo) {
                ...
                case 'Empresa':
                    ...
                    setTextareaValue('mision', userData.mision);
                    setTextareaValue('vision', userData.vision);
                    setTextareaValue('objetivos', userData.objetivos);
                    break;
